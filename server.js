...
async function getUser(id) {
  if (pool) {
    const result = await pool.query(`SELECT id,nickname,password,role,description,avatar,posts,topics,level,battle_pass AS "battlePass",blocked,created_at AS "createdAt" FROM users WHERE id=$1`, [id]);
...
async function getBP(userId){
  if(pool){const result=await pool.query(`SELECT user_id AS "userId",xp,claimed,premium_claimed AS "premiumClaimed",premium FROM battlepass WHERE user_id=$1`,[userId]);if(result.rows[0])return result.rows[0];const created=await pool.query(`INSERT INTO battlepass(user_id) VALUES($1) RETURNING user_id AS "userId",xp,claimed,premium_claimed AS "premiumClaimed",premium`,[userId]);return created.rows[0];}
  const rows=read('battlepass');let state=rows.find(i=>Number(i.userId)===Number(userId));if(!state){state={userId,xp:0,claimed:[],premiumClaimed:[],premium:false};rows.push(state);write('battlepass',rows);}return state;
}
async function saveBP(state){
  if(pool){await pool.query(`INSERT INTO battlepass(user_id,xp,claimed,premium_claimed,premium) VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id) DO UPDATE SET xp=EXCLUDED.xp,claimed=EXCLUDED.claimed,premium_claimed=EXCLUDED.premium_claimed,premium=EXCLUDED.premium`,[state.userId,Number(state.xp)||0,Array.isArray(state.claimed)?state.claimed:[],Array.isArray(state.premiumClaimed)?state.premiumClaimed:[],Boolean(state.premium)]);return;}
  const rows=read('battlepass'),index=rows.findIndex(i=>Number(i.userId)===Number(state.userId));if(index>=0)rows[index]=state;else rows.push(state);write('battlepass',rows);
}
app.get('/api/battlepass',auth,async(req,res)=>{try{const state=await getBP(req.user.id),xp=Math.max(0,Math.min(2000,Number(state.xp)||0));res.json({xp,level:Math.min(20,Math.floor(xp/100)+1),claimed:Array.isArray(state.claimed)?state.claimed:[],premiumClaimed:Array.isArray(state.premiumClaimed)?state.premiumClaimed:[],premium:Boolean(state.premium)});}catch(error){console.error('Battle Pass load error:',error);res.status(500).json({error:'Ошибка загрузки Battle Pass'});}});
app.post('/api/battlepass/claim',auth,async(req,res)=>{const level=Number(req.body.level),premium=Boolean(req.body.premium);if(!Number.isInteger(level)||level<1||level>20)return res.status(400).json({error:'Недопустимый уровень'});try{const state=await getBP(req.user.id),currentLevel=Math.min(20,Math.floor((Number(state.xp)||0)/100)+1);if(level>currentLevel)return res.status(403).json({error:'Награда ещё заблокирована'});if(premium&&!state.premium)return res.status(403).json({error:'Premium не активирован'});const key=premium?'premiumClaimed':'claimed';state[key]=Array.isArray(state[key])?state[key]:[];if(state[key].includes(level))return res.status(409).json({error:'Награда уже получена'});state[key].push(level);await saveBP(state);res.json({ok:true,level,premium});}catch(error){console.error('Battle Pass claim error:',error);res.status(500).json({error:'Не удалось получить награду'});}});
...
app.post('/api/topics/:id/close',auth,async(req,res)=>{if(!isStaff(req.user))return res.status(403).json({error:'Недостаточно прав'});try{if(pool){const result=await pool.query('UPDATE topics SET closed=NOT closed,updated_at=$2 WHERE id=$1 RETURNING closed',[req.params.id,new Date().toISOString()]);if(!result.rows[0])return res.status(404).json({error:'Тема не найдена'});return res.json({ok:true,closed:result.rows[0].closed});}const topics=read('topics'),topic=topics.find(i=>String(i.id)===String(req.params.id));if(!topic)return res.status(404).json({error:'Тема не найдена'});topic.closed=!topic.closed;topic.updatedAt=new Date().toISOString();write('topics',topics);res.json({ok:true,closed:topic.closed});}catch(error){console.error('Topic close error:',error);res.status(500).json({error:'Не удалось изменить статус темы'});}});
