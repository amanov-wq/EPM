...
async function initDatabase() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      posts INTEGER NOT NULL DEFAULT 0, topics INTEGER NOT NULL DEFAULT 0, level INTEGER NOT NULL DEFAULT 1, blocked BOOLEAN NOT NULL DEFAULT FALSE,
...
async function loadToJson(name) {
  if (!pool) return null;
  if (name === 'users') { const {rows}=await pool.query('SELECT id,nickname,password,role,description,avatar,posts,topics,level,blocked,created_at AS "createdAt" FROM users ORDER BY id'); return rows; }
