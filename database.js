  ...
  try {
    ...
    if (name === 'users') {
      ...
      for (const t of rows) {
        await client.query(
          `INSERT INTO topics
            (id,title,content,author,author_id,category,pinned,closed,views,replies_count,created_at,updated_at)
          ...
          [
            ...
            t.updatedAt || t.createdAt || new Date()
          ]
        );
      }
      ...
      await client.query(
        `SELECT setval(pg_get_serial_sequence('topics','id'),
          COALESCE((SELECT MAX(id) FROM topics),1), true)`
      );
    } else if (name === 'replies') {
      if (ids.length) {
        await client.query(`DELETE FROM replies WHERE id <> ALL($1::int[])`, [ids]);
      } else {
      ...
      for (const reply of rows) {
        await client.query(
          `INSERT INTO replies
            (id,topic_id,content,author,author_id,created_at)
           VALUES ($1,$2,$3,$4,$5,$6)
           ON CONFLICT (id) DO UPDATE SET
             content=EXCLUDED.content,
             author=EXCLUDED.author,
      ...
      await client.query(
        `SELECT setval(pg_get_serial_sequence('replies','id'),
          COALESCE((SELECT MAX(id) FROM replies),1), true)`
      );
    }
...
async function loadToJson(name) {
  ...
  if (name === 'topics') {
    const { rows } = await pool.query(
      'SELECT id,title,content,author,author_id AS "authorId",category,pinned,closed,views,replies_count AS "repliesCount",created_at AS "createdAt",updated_at AS "updatedAt" FROM topics ORDER BY id'
    );
    return rows;
  }
  ...
  if (name === 'replies') {
    const { rows } = await pool.query(
      'SELECT id,topic_id AS "topicId",content,author,author_id AS "authorId",created_at AS "createdAt" FROM replies ORDER BY id'
    );
    return rows;
  }
