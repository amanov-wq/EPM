...
async function initDatabase() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      battle_pass INTEGER NOT NULL DEFAULT 0, blocked BOOLEAN NOT NULL DEFAULT FALSE,
...
async function loadToJson(name) {
  if (!pool) return null;
  if (name === 'users') { const {rows}=await pool.query('SELECT id,nickname,password,role,description,avatar,posts,topics,level,battle_pass AS "battlePass",blocked,created_at AS "createdAt" FROM users ORDER BY id'); return rows; }
