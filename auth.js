...
function saveSession(data){if(!data?.token||!data?.user)return;localStorage.setItem(TOKEN_KEY,data.token);localStorage.setItem(USER_KEY,JSON.stringify(data.user));localStorage.setItem('user',JSON.stringify(data.user))}
